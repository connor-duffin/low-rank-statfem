...
# plotting variables used throughout the entire program
t = model["t"][:]
x_u = model["x_u"][:]
x_v = model["x_v"][:]
...
for i_plot, i_array in enumerate(idx_data):
    ...
    axs[0, i_plot].plot(x_u, u_curr, label=r"$\mathbf{m}_n^u$")
    ...
    axs[0, i_plot].plot(x_v, v_curr, label=r"$\mathbf{m}_n^v$")
    ...
    axs[1, i_plot].plot(x_u, u_curr, label=r"$\mathbf{m}_n^u$")
    axs[1, i_plot].fill_between(x_u.flatten(),
                                u_curr - 2 * u_sd,
                                u_curr + 2 * u_sd,
                                alpha=0.2)

    axs[1, i_plot].plot(x_v, v_curr, label=r"$\mathbf{m}_n^v$")
    axs[1, i_plot].fill_between(x_v.flatten(),
                                v_curr - 2 * v_sd,
                                v_curr + 2 * v_sd,
                                alpha=0.2)

    axs[1, i_plot].plot(x_u, u_prior_curr, label=r"$\mathbf{m}_n^u$ (prior)")
    axs[1, i_plot].plot(x_v, v_prior_curr, label=r"$\mathbf{m}_n^v$ (prior)")
...
axs[0].set_title(r"$\mathbf{m}^u(x, t)$")
...
axs[1].set_title(r"$\mathbf{m}^v(x, t)$")
...
axs[1].set_ylabel(
    r"$\Vert \mathrm{var}(\mathbf{u}_n) - \mathrm{var}(\mathbf{u}_{n, LR})" +
    r" \Vert / \Vert \mathrm{var}(\mathbf{u}_n) \Vert$",
    fontsize="small")
